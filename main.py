...
def get_weather(region):
    ...
    region_url = "https://devapi.qweather.com/v7/city/lookup?location={}&key={}".format(region, key)
...
def send_message(to_user, access_token, region, weather, temp, wind_dir, min_temp, max_temp, sunrise, sunset, note_ch, note_en):
    ...
    try:
        if "birthday1" in config:
            ...
            if b1_days == "0":
                b1_text = f"🎉{config['birthday1']['name']}生日快乐！天天开心，万事顺意！"
            else:
                b1_text = f"{config['birthday1']['name']}生日还有{b1_days}天"
        ...
        if "birthday2" in config:
            ...
            if b2_days == "0":
                b2_text = f"🎉{config['birthday2']['name']}生日快乐！平安喜乐，得偿所愿！"
            else:
                b2_text = f"{config['birthday2']['name']}生日还有{b2_days}天"
