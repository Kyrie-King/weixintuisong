...
# ======================= 【新增】判断今天是否生日 =======================
def is_today_birth(birthday_str, year, today):
    try:
        if birthday_str.startswith("r"):
            _, month, day = birthday_str.split("-")
            lunar_date = ZhDate(year, int(month), int(day))
...
# ======================= 【新增】生日祝福文案 =======================
def get_birth_wish(name):
    wishes = [
        f"🎂 亲爱的{name}，生日快乐！我会一直陪着你，岁岁年年，朝朝暮暮～",
        f"✨ 祝我的宝贝{name}生日快乐！愿你被世界温柔以待，被我永远偏爱～",
        f"💖 {name}生日快乐！谢谢你来到我的世界，往后每一天，我都在～",
        f"🥳 恭喜我的{name}又长大一岁！愿你平安喜乐，万事顺心，永远做我的小朋友～",
        f"🌟 亲爱的{name}，生日快乐！余生漫漫，我只想和你一起走～",
        f"💞 {name}生日快乐！愿所有美好与幸运，都奔你而来～",
        f"🎈 祝我最爱的{name}生日快乐！有我在，你永远不用长大～"
    ]
...
def send_message(to_user, access_token, region, weather, temp, wind_dir, min_temp, max_temp, sunrise, sunset, note_ch, note_en):
    ...
    # ======================= 【核心】判断是否使用生日模板 =======================
    today_is_birth = False
    birth_wish = ""
    b1_is_birth = is_today_birth(config["birthday1"]["birthday"], today.year, today)
    b2_is_birth = is_today_birth(config["birthday2"]["birthday"], today.year, today)

    if b1_is_birth:
        today_is_birth = True
        birth_wish = get_birth_wish(config["birthday1"]["name"])
    if b2_is_birth:
        today_is_birth = True
        birth_wish = get_birth_wish(config["birthday2"]["name"])

    # 选择模板
    if today_is_birth:
        template_id = config["birthday_template_id"]
        print("🎂 今天是生日！使用生日模板")
    else:
    ...
    # 生日祝福（仅生日模板传入）
    if today_is_birth:
        data["data"]["birth_wish"] = {"value": birth_wish, "color": "#FF6600"}
...
if __name__ == "__main__":
    ...
    must_have = ["app_id", "app_secret", "weather_key", "template_id", "birthday_template_id", "user", "region", "love_date"]
